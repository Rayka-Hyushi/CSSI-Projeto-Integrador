...
public interface RecomendacaoRepository extends JpaRepository<Recomendacao, Long> {
    ...
    Optional<Recomendacao> findByClienteAndPrestador(@Param("clienteId") Long clienteId,
            @Param("prestadorId") Long prestadorId);

    @Modifying
    @Query("DELETE FROM Recomendacao r WHERE r.prestador.id = :prestadorId")
    void deleteByPrestadorId(@Param("prestadorId") Long prestadorId);

    @Modifying
    @Query("DELETE FROM Recomendacao r WHERE r.cliente.id = :clienteId")
